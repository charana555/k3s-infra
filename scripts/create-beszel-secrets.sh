...
# Contains: hub admin credentials + agent KEY/TOKEN for auto-registration
# Usage: ./create-beszel-secrets.sh
#
# Two-phase flow:
#   Phase 1 (first run):  Provide admin email + password only.
#                         Key/token can be left empty - apply the hub, create a
#                         universal token in the UI, then re-run this script.
#   Phase 2 (second run): Provide admin email + password + KEY + TOKEN from the hub UI.
...
echo ""
echo "Get these from: Beszel Hub -> Settings -> Tokens -> Create Universal Token"
...
if [ -z "$AGENT_KEY" ] || [ -z "$AGENT_TOKEN" ]; then
    ...
    echo "4. Hub UI -> Settings -> Tokens -> Create Universal Token"
    ...
    echo "2. Verify agents registered:       kubectl get pods -n monitoring -l app=beszel-agent -o wide"
    echo "3. Check hub UI - all nodes should appear as systems"
    echo "4. Hub UI -> Notifications -> add Discord webhook"
fi
